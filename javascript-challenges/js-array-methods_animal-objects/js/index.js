...
const allAnimalsInEuropeWeighLessThanOnehundred = animals
  .filter((animal) => animal.continent === "Europe") // 1 continent
  .every((animal) => animal.weight < 100); // 2 weigth

// Hint: filter + map + reduce
const weightOfAllAnimalsInAfrica = animals
  .filter((animal) => animal.continent === "Africa")
  .map((animal) => animal.weight)
  .reduce((a, b) => a + b);

// Hint: As above, but divided by the number of animals in Africa.
const animalsInAfrica = animals.filter(
  (animal) => animal.continent === "Africa"
);

const averageWeightOfAllAnimalsInAfrica =
  animalsInAfrica.reduce((a, b) => a + b.weight, 0) / animalsInAfrica.length;
